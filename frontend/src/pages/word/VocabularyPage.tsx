    ...
    <div className="min-h-screen flex flex-col bg-[#f8fafc]">
      <div className="mx-auto flex w-full max-w-275 flex-col gap-6 px-4 py-8 sm:px-6 sm:py-10 lg:px-8 lg:py-12">
        ...
        <header className="flex flex-col gap-2">
          ...
          {/* Editable title */}
          <div className="mt-4 inline-flex w-full max-w-md items-center gap-2 rounded-xl border border-slate-200 bg-white px-3.5 py-2 shadow-sm focus-within:border-blue-400 focus-within:ring-4 focus-within:ring-blue-100">
            <Input
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              readOnly={!editingTitle}
              placeholder="제목을 입력하세요"
              className="h-8 flex-1 border-0 bg-transparent p-0 text-slate-800 shadow-none focus-visible:ring-0 focus-visible:ring-offset-0"
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative flex-1 sm:max-w-md">
              ...
              <Input
                ...
                placeholder="단어 검색"
                className="h-11 rounded-xl border-slate-200 bg-slate-50/40 pl-10 focus-visible:border-blue-400 focus-visible:bg-white focus-visible:ring-4 focus-visible:ring-blue-100"
              />
            </div>
            <button
              type="button"
              onClick={() => setSortDesc((v) => !v)}
              className="inline-flex h-10 items-center justify-center rounded-lg border border-slate-200 bg-white px-3.5 text-slate-600 transition hover:bg-slate-50 hover:text-slate-900"
              style={{ fontSize: "13px" }}
            >
              {sortDesc ? "최신순" : "오래된순"}
            </button>
            ...
            {filtered.length === 0 ? (
              ...
              <ul className="divide-y divide-slate-100">
                {filtered.map((item) => (
                  <li
                    key={item.id}
                    className="group flex flex-col gap-4 rounded-xl px-2 py-5 transition hover:bg-slate-50/60 sm:flex-row sm:items-start sm:justify-between sm:px-4"
                  >
                    ...
                    <div className="flex items-center justify-between gap-3 sm:flex-col sm:items-end sm:justify-start">
                      ...
                      <button
                        ...
                        aria-label="단어 삭제"
                        className="inline-flex items-center gap-1 rounded-md px-2 py-1 text-slate-400 transition hover:bg-red-50 hover:text-red-500"
                        style={{ fontSize: "12px" }}
                      >
