...
import com.heesik.backend.global.config.security.CustomUserDetails;
...
@Tag(name = "사용자 정보 API", description = "사용자 정보 CRUD API (JWT 필요)")
public class UserController {
    ...
    public ResponseEntity<TokenResDTO> changeName(@RequestBody UpdateNameReqDTO request,
                                                  @AuthenticationPrincipal CustomUserDetails userDetails,
                                                  HttpServletResponse response) {
        TokenPair token = userService.updateUserName(request.name(), userDetails.id());
